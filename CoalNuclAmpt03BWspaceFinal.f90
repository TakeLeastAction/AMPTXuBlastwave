	...
	dimension xpv(Njobmax),xdv(Njobmax),xppv(Njobmax),xtv(Njobmax)
	   ...
	   xkm=0.
       nevent = 50
       write(*,*)"data reading begin:"
       CALL DataRead(nevent,njobmax,xpro,xneu,ipro,ineu,xnp,xnn,xpionp,xpionm,xkaonp,xkaonm)
       write(*,*)"data reading done!"
       Njobt = 1	   
	   neventd=Nevent/njobt
               ncoal = 0
	open (unit=20, file='yieldv.dat', status='unknown')

	open (unit=21, file='yield.dat', status='unknown')	
	!open (unit=21, file='yieldv.dat', status='unknown')
	do njob=1,njobt
       ncoal = ncoal+1
	   nbegin = (njob-1)*neventd+1
       nend = njob*neventd
	   
	   !write(*,*)"dear master: ",ineu(1:nevent)
       sf = 3.2/2.0**0.5	   
	   CALL COALdeu(nbegin,nend,xpro,xneu,ipro,ineu,sf,xnd) 
	   write(*,*)"d/p ratio:",xnd/xnp
	   rms = 1.76
	    !nevent=20
       rapcut = 10.0
	   CALL COALhe3(nbegin,nend,xpro,xneu,ipro,ineu,rms,xhe3,rapcut)
	 ...
	 xkm = xkm/ncoal
       ...
       write(21,101)xpp,xpm,xkp,xkm,xp,xn,xd,xt
	 ...
	 write(*,*)"he-3 p/d^2 ratio:",xt*xp/xd**2.*xn/xp
         write(*,*)"density moments ratio:",xt*xp/xd**2.*xn/xp*2.0*3.0**0.5
...
101 format((1x,I6),8(1x,F10.5))	  
	  ...
	  SUBROUTINE COALdeu(nbegin,nend,xpro,xneu,ipro,ineu,sf,avnd)
	  implicit real*8 (a-h,o-z)
	  integer,parameter :: NumPar= 1000, Ntest=1000
	  dimension xpro(8,NumPar,Ntest),xneu(8,NumPar,Ntest)
	  dimension ipro(Ntest),ineu(Ntest)
...
101 format((1x,I6),8(1x,F10.5))	  
	  ...
	  SUBROUTINE COALhe3(nbegin,nend,xpro,xneu,ipro,ineu,rms,avt,rapcut)
	  implicit real*8 (a-h,o-z)
	  integer,parameter :: NumPar= 1000, Ntest=1000
	  dimension xpro(8,NumPar,Ntest),xneu(8,NumPar,Ntest)
	  dimension ipro(Ntest),ineu(Ntest)
	  ...
	  gc = 1./4.
	  nparak=0
       do npara=nbegin,nend-2
	   do npara1=npara+1,nend-1
	   do npara2=npara1+1,nend
		  xnt=0.
		  nparak=nparak+1
        ...
        do k1=1,ipro(npara)
			...
			xm1 = xpro(8,k1,npara)
			xE1 = sqrt(xm1**2.+px1**2.+py1**2.+pz1**2.)
            rap1 = 0.5*log((xE1+pz1)/(xE1-pz1))
		!if(abs(rap1)>rapcut) cycle			
			do k2=1,ipro(npara1)
			...
			xE2 = sqrt(xm2**2.+px2**2.+py2**2.+pz2**2.)
            rap2 = 0.5*log((xE2+pz2)/(xE2-pz2))	
		!if(abs(rap2)>rapcut) cycle				
		   do k3=1,ineu(npara2)
            ...
            rap3 = 0.5*log((xE3+pz3)/(xE3-pz3))
		!if(abs(rap3)>rapcut) cycle				
			px=px1+px2+px3
			 py=py1+py2+py3
			 pz=pz1+pz2+pz3
			 xmass=xm1+xm2+xm3
			...
			py3new = py3
			pz3new = pz3
			xE3new = sqrt(xm3**2.+px3new**2.+py3new**2.+pz3new**2.)
			...
			w = gc*8.**2.*exp(-disx**2./sig1**2.-disp**2.*sig1**2./hbc2-disx1**2./sig2**2.-disp1**2.*sig2**2./hbc2)
                        !w = w/(2*pi*sig1**2.*2*pi*sig2**2.)**1.5
		...
		!xntv(nparak) = xnt
		avnt = avnt+xnt
                !avnt = avnt/(2*3.14*sig1**2.0*2*3.14*sig2**2.0)**1.5
	   enddo
	   enddo
	   ...
	   avt = avnt/float(nparak)
           avt = avt/(2.0*3.14*sig1**2.0*3.0/2.0*3.14*sig2**2.0)**1.5
           avt = avt*3**1.5
